...
class PlatformCredentialsService:
    ...
    def admin_api_key(self) -> str:
        key = settings.gateway_admin_key or settings.platform_api_key
        if not key:
            raise RuntimeError(
                "Configure STUDIO_GATEWAY_ADMIN_KEY (bootstrap admin) to provision lab credentials."
            )
        return key
    ...
    async def provision(self, lead_id: str, *, force: bool = False) -> PlatformCredentials:
        ...
        manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
        gateway_base = resolve_urls(manifest).gateway_base_url.rstrip("/")
